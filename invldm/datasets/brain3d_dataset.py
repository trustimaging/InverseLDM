...
class Brain3DDataset(BaseDataset):
    def __init__(self, args):
        ...
        assert not self.args.sampling_only, "Brain3DDataset is designed for training. Please see Brain3DSamplingDataset"
    ...
    def _get_image_paths(self, path, maxsamples=None):
        ...
        for dirpath, dirname, filenames in os.walk(path):
            filenames = [f for f in filenames if f.lower().startswith(prefix) and f.lower().endswith((suffix))]
            for f in filenames:
                self.data_paths.append(os.path.join(dirpath, f))
    ...
    def __getitem__(self, index):
        ...
        # Perform resizing in 3d here
        if self.args.resize is not None:
            self.transform = [Resize(self.args.resize)] + self.transform[1:]
    ...
    def info(self, nsamples=None):
        ...
        arr = torch.empty_like(sample).unsqueeze(0).repeat(nsamples, 1, 1, 1, 1)
