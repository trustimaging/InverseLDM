...
def update_config_for_view(config_path, view_type, output_config_path):
    """
    ...
    config['autoencoder']['log_path'] = base_log_path.replace('view_conditioning', f'view_conditioning_{view_type}')
    config['autoencoder']['ckpt_path'] = config['autoencoder']['ckpt_path'].replace('view_conditioning', f'view_conditioning_{view_type}')
    config['autoencoder']['recon_path'] = config['autoencoder']['recon_path'].replace('view_conditioning', f'view_conditioning_{view_type}')
    
    config['diffusion']['log_path'] = config['diffusion']['log_path'].replace('view_conditioning', f'view_conditioning_{view_type}')
    config['diffusion']['ckpt_path'] = config['diffusion']['ckpt_path'].replace('view_conditioning', f'view_conditioning_{view_type}')
    config['diffusion']['samples_path'] = config['diffusion']['samples_path'].replace('view_conditioning', f'view_conditioning_{view_type}')
...
def train_view(config_path, view_type, experiment_name, gpu_ids, overwrite=False):
    """
    ...
    view_config_path = f"view_conditioning_{view_type}_config.yml"
    update_config_for_view(config_path, view_type, view_config_path)
...
def main():
    ...
    for view in args.views:
        ...
        if args.sequential:
            ...
            success = train_view(args.config, view, args.name, args.gpu_ids, args.overwrite)
            if success:
            ...
            view_config_path = f"view_conditioning_{view}_config.yml"
            update_config_for_view(args.config, view, view_config_path)
